# convert current.data file to a root file
#
# run this script
#
...
#----------------------------------------------------------------------
# adjust if you know what you are doing
#----------------------------------------------------------------------
...
ret = run_command(command, 'out1', 'err1',shell = True)
if (ret != 0):
    print "problem doing: " + command
    print "Check : err1"
    sys.exit(-1)
# command = SIMDIR+'root/make_tree < make_root_tmp.dat'
command = SIMDIR+'root/make_tree'
ret = run_command(command,'out2','err2',stdin=open('make_root_tmp.dat'))
if (ret != 0):
    print "problem doing: " + command
    print "Check : err2"
    sys.exit(-2)
...
ret = run_command(command,'out3','err3', shell = True)
if (ret != 0):
    print "problem doing: " + command
    print "Check : err3"
    sys.exit(-3)
